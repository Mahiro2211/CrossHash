...
class CrossFormerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f'H W is {H} {W}')
        # print(f'\tcrossblk x shape is {x.shape}')
        # exit()
        
        shortcut = x
        x = self.norm1(x)
        x = x.view(B, H, W, C)
        # print(f'before group embedding {x.shape}')
...
class CrossFormer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=[4], in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, merge_size=[[2], [2], [2]],hash_bit=0, **kwargs):
        super().__init__()
        ...
        self.mlp_ratio = mlp_ratio
        self.hash_layer = HashLayer(hash_bit=hash_bit)
        ...
        self.head = nn.Linear(self.num_features, hash_bit)
